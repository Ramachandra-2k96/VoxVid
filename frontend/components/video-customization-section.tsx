...
const customizationOptions: CustomizationOption[] = [
  {
    id: "professional",
    question: "Show me professional voice options",
  ...
  {
    id: "casual",
    question: "Find casual, friendly avatars",
  ...
  {
    id: "educational",
    question: "Browse educational video templates",
  ...
  {
    id: "business",
    question: "Show business presentation styles",
...
interface VideoCustomizationSectionProps {
}

export default function VideoCustomizationSection({ }: VideoCustomizationSectionProps) {
  const [activeQuery, setActiveQuery] = useState(customizationOptions[0])
  ...
  useEffect(() => {
    const preloadVideos = async () => {
      ...
      setVideosLoaded({ [customizationOptions[0].id]: true })

      // Then preload the rest in the background
      for (const query of customizationOptions) {
        if (query.id === customizationOptions[0].id) continue
  ...
  const handleQueryClick = (query: CustomizationOption) => {
    setActiveQuery(query)
  }

  // Simplified hover handling - no delay
  const handleQueryHover = (query: CustomizationOption) => {
    if (isHovering) {
  ...
  return (
    <div className="my-24">
      ...
      <div className="text-center mb-6 md:mb-12 px-4">
        ...
        >
          Ask anything. VoxVid understands your creative vision — from script to final video.
        </p>
      </div>
          ...
          <div className="relative z-10 pt-4 sm:pt-12 md:pt-16 pb-0 px-4 sm:px-6 md:px-12">
            ...
            <div className="rounded-t-lg overflow-hidden shadow-2xl max-w-4xl mx-auto border border-white/10 border-b-0 relative">
              <FeatureVideo
                ...
                alt={`Video Customization: ${activeQuery.question}`}
              ...
              <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 lg:hidden z-20">
                <div className="flex gap-2 bg-black/50 backdrop-blur-sm rounded-full px-3 py-2">
                  {customizationOptions.map((query, index) => (
                    <button
                      key={query.id}
                ...
                <div className="flex gap-4 lg:gap-8 items-end justify-between">
                  ...
                  <div className="flex flex-col gap-2 max-w-xs">
                    ...
                    <div className="flex flex-col gap-2 w-[240px]">
                      {customizationOptions.map((query) => (
                        <button
                          key={query.id}
